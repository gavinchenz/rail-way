        ...
        <Row>
          ...
          <Col  span="6" style="text-align: right">
            <FormItem :label-width="50">
              <Button @click="addPower" type="success"  icon="plus" title="新增权限" size="small" :disabled="!$store.state.auth.has('1-1-1-13')">新增</Button>
            </FormItem>
          </Col>
        </Row>
...
<script>
  export default {
    ...
    methods: {
      ...
      search(){
        if (!!this.searchInfo.operGroup.gname) {
          this.$emit('search', this.searchInfo.operGroup.gname);
        } else {
      ...
      resat() {
        this.searchInfo.operGroup.gname = ''
        this.$emit('search', this.searchInfo.operGroup.gname);
      }
    }
  }
