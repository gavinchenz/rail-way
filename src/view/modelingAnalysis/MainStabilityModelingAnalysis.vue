<template>
    <div class="vue-body">
                ...
                <Row :gutter="24">
                    ...
                    <Col span="22" class="col-checkbox">
                        <CheckboxGroup v-model="searchData.abnormalType">
                            ...
                            <Checkbox label="长往返维稳案件多发地人员分析">长往返维稳案件多发地人员分析</Checkbox>
                            <Checkbox label="涉嫌维稳案件人员乘车一览">涉嫌维稳案件人员乘车一览</Checkbox>
                        </CheckboxGroup>                
                    </Col>
                </Row>
            </FormItem> 
        ...
        <!-- 表格和翻页 -->
        <div class="table-box">
            <Table :loading="loading" class="table-content" border :columns="columns" :data="mainStabilityModelingDataList" @on-sort-change="sortClick"></Table>
...
<script>
    import {getMainStabilityModelingDataList} from '@/service/getData';//异步请求链接
    ...
    export default {
        ...
        methods:{
            ...
            async getListData(sendData) {
                this.loading = true;
                await getMainStabilityModelingDataList(sendData)
                .then(res => {
                    this.loading = false;
                    this.mainStabilityModelingDataList = res.data;
                    console.log(this.mainStabilityModelingDataList);
