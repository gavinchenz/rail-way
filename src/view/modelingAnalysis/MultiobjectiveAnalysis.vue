   ...
   <div class="vue-body">
    <!-- 面包屑 -->
        <Breadcrumb class="router-box">
            ...
            <BreadcrumbItem>多目标分析</BreadcrumbItem>
        </Breadcrumb>
        <!-- 查询条件 -->
        ...
        <!-- 表格和翻页 -->
        <div class="table-box">
            <Table :loading="loading" class="table-content" border :columns="columns" :data="multiObjectiveDataList" @on-sort-change="sortClick"></Table>
...
<script>
    import {getmultiObjectiveList} from '@/service/getData';//异步请求链接
...
export default {
    ...
    data () {
        return {
            ...
            columns: [
                ...
                {title:'身份证号',width: 600,align: 'center',key:'sfzh',sortable: true},
                {title:'共同乘车次数',width: 200,align: 'center',key:'gtcccs',sortable: true},
                {title:'共同行踪轨迹',width: 800,align: 'center',key:'gtxzgj',sortable: true},
                {
                    title: '操作',
                    width: 150,
                    align: 'center',
    ...
    methods:{
            ...
            async getListData(sendData) {
                this.loading = true;
                await getmultiObjectiveList(sendData)
                .then(res => {
                    this.loading = false;
                    this.multiObjectiveDataList = res.data;
                    console.log(this.multiObjectiveDataList);
